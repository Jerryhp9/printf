...
int format_specifiers(char format, va_list *list)
{
	if(format == 'c')
		...
		return (ft_print_digit(va_arg(*list, long), "0123456789"));
	else if(format == 'i')	
...
int	ft_printf(const char *format, ...)
{
	...
	while(format[i])
	{
		if(format[i] == '%') //implementing where the next index is not null means its a format specifier
		{	
			i++;
			if(format[i])
				count += format_specifiers(format[i], &list); //function call into retrieving the definitive format specifier
		}
		else
